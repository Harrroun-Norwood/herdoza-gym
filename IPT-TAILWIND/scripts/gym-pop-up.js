...
const gym_payment_summary = [
  ...
  },
  {
    ...
    pass: "Regular",
    description: "Get full access to our gym equipment for as long as you stay in gym days.",
    price: 40,
    duration: 1,
  },
  {
    ...
    description: "Single training session with our professional fitness expert.",
    price: 100,
    duration: 1,
    hasTrainer: true,
  },
...
function openGymMembershipPopup(item) {
  const typeKey = item.hasTrainer ? 
    (item.duration === 1 ? 'trainerSingle' : 'trainer25') :
    (item.duration === 30 ? 'full' : item.duration === 15 ? 'half' : 'regular');
    
  // Save selected membership data temporarily
  localStorage.setItem('selectedMembershipData', JSON.stringify({
    type: "Gym Fitness",
