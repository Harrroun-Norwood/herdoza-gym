...
async function getUserBookings() {
    try {
        ...
        const bookings = {
            mmaPerSession: JSON.parse(localStorage.getItem("mmaPerSessionBookings") || "[]").filter(filterValid),
            mmaBulkSession: JSON.parse(localStorage.getItem("mma25SessionUserBookings") || "[]").filter(filterValid),
            mmaZumba: JSON.parse(localStorage.getItem("mmaZumbaBookings") || "[]").filter(filterValid),
            zumba: JSON.parse(localStorage.getItem("zumbaBookings") || "[]").filter(filterValid),
            studio: [
                ...JSON.parse(localStorage.getItem("smallStudioBookings") || "[]"),
                ...JSON.parse(localStorage.getItem("largeStudioBookings") || "[]")
            ].filter(filterValid),
            gym: JSON.parse(localStorage.getItem("gymMembershipBookings") || "[]").filter(filterValid)
        };
...
function storeBookingInLocalStorage(booking) {
    try {
        ...
        const adminBookings = JSON.parse(localStorage.getItem("adminBookingsData") || "[]");
        const adminBookingIndex = adminBookings.findIndex(b => b.id === booking.id);
...
function removeBookingFromLocalStorage(bookingId) {
    ...
    const adminBookings = JSON.parse(localStorage.getItem("adminBookingsData") || "[]");
    const updatedAdminBookings = adminBookings.filter(booking => booking.id !== bookingId);
    localStorage.setItem("adminBookingsData", JSON.stringify(updatedAdminBookings));
...
async function getBookingById(bookingId) {
    try {
        ...
        // Fallback to localStorage
        const bookings = JSON.parse(localStorage.getItem("adminBookingsData") || "[]");
